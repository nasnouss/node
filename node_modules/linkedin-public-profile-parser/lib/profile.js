...
var allKeyWord;
  ...
  console.log("image url : " + $( ".image .photo .lazy-loaded" ).get( 0 ))
  ...
  data.experience = {all:[] ,current:[], past:[]}; // empty if none listed
  var experience = [];
  var positions = $('.position');
  if(positions.length > 0) {
    // variable qui va contenir tout le texte sur lequel on va faire la recherche
    var text;
    ...
    for(i = 1; i <= positions.length; i++) {
      ...
      exp.desc = ent.decode($('.position:nth-child('+i+') .description').html() + "").replace(/\\/g, "").replace("\\", "");;
      experience.push(exp);
    }
  }
  data.experience.current = experience.filter(function(exp){
    return exp.date.match("Present" || "Aujourd’hui");
  });
  data.experience.past = experience.filter(function(exp){
    return exp.date.indexOf('Present') === -1;
  })
// contient toutes les expéreiences de l'utilisateur 
...
if (schools.length > 0){
  for(i = 1; i <= schools.length; i++) {
   ...
   // console.log(" nom ecole : " + $('.school:nth-child('+i+') h4.item-title a').text())
    ...
    if (school1 !== ""){
      console.log(school1);
    sch.schoolName = school1;
    sch.lastSchoolName = true;
